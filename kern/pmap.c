...
page_init(void)
{
	...
	// free pages!
	size_t i;
	int no_pages_in_IOhole = ((uint32_t)(EXTPHYSMEM - IOPHYSMEM)) / PGSIZE;
	int no_pages_in_extended_used = ((uint32_t)boot_alloc(0) - KERNBASE)/ PGSIZE;
	
	//Physical page 0 is in use for preserving real-mode IDT and BIOS stuctures
	pages[0].pp_ref = 1;
	...
	//(IO hole + pages already used in kernel) which must never be re-allocated
	for(i = npages_basemem; i < npages_basemem + no_pages_in_IOhole + no_pages_in_extended_used; i++)
  		pages[i].pp_ref = 1;

	//Rest of extended kernel memory
	for (; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
